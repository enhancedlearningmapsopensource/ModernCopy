...
function Delete($id){
    ...
    // Perform deletion
    $database->PrototypeQueryQuiet("DELETE FROM ELM_NODESTANDARD WHERE SID=? AND NODEID=?", array(&$sid, &$nodeID));
    
    // Verify deletion
    $vresult = $database->PrototypeQuery("SELECT * FROM ELM_NODESTANDARD WHERE SID=? AND NODEID=?", array(&$sid, &$nodeID));
...
function GetAll(){
    ...
    if($editModeOn){
        $query = "SELECT * FROM ELM_NODESTANDARD";
    }else{
...
function Post($data){
    ...
    $database->PrototypeInsert("INSERT INTO ELM_NODESTANDARD(SID,NODEID,DATE,D) VALUES (?,?,?,?)", 
            array(
                &$sid,          // SID
                &$nodeid,       // NODEID
                &$date,         // DATE
